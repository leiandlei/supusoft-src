...
class Wechat
{
    ...
    public function log($log){
		if ($this->debug) {
    		...
    		if(!is_readable($dir))chmod($dir, 0777);

			if( file_exists($dir.$file) ){
				file_put_contents($dir.$file,$content.$log.$content,FILE_APPEND);
			}else{
				file_put_contents($dir.$file,$content.$log.$content);
			}
			return $dir.$file;
		}
		return false;
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function setCache($cachename,$value,$expired){
		$logPath   = './logs';if(!file_exists($logPath))@mkdir($logPath, 0777,true);
		$cachename = rtrim($logPath, '/').'/'.$cachename.'.log';
		file_put_contents($cachename,json_encode(array('openid'=>$value,'time'=>time()+$expired)));
		return true;
	}
	...
	protected function getCache($cachename){
		$cachename = './logs/'.$cachename.'.log';
		if(!file_exists($cachename))return false;
		$cache = file_get_contents($cachename);
		$cache = empty($cache)?array():json_decode($cache,true);
		if( !empty($cache)&&(time()-$cache['time'])< 0 ){
			return $cache['openid'];
		}
		return false;
	}
