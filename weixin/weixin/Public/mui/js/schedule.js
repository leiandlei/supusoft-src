...
;(function (undefined) {
	...
	//日期格式化
	function formartDate (y,m,symbol) {
		symbol = symbol || '-';
		m = (m.toString())[1] ? m : '0'+m;
		var formartArr = [y,m];
		return formartArr.join('-');
	}

	function Schedule (opt) {
		var def = {},
			opt = extend(def,opt,true),
			curDate = opt.date ? new Date() : new Date(),
		...
		var init = function () {   
			var scheduleHd = '<div class="schedule-hd">'+
								...
								'</div>'+
								'<div class="today">'+formartDate(year,month,'-')+'</div>'+
								'<div>'+
			...
			var cover = '<div style="position: absolute;top: 0;bottom: 0;left: 0;right: 0;z-index: 100;"></div>';
			el.innerHTML = scheduleHd + scheduleWeek + scheduleBd +cover;
			render();
		}
		var render = function () {
