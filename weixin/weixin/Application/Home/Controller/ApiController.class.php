...
class ApiController extends Controller {
    ...
    {
        $arrActionData['unionToken'] = !empty($arrActionData['unionToken'])?$arrActionData['unionToken']:session(SANDC_KEY.'.uniontoken');
        if(empty($arrHeader)){
            global $arrHeader;
            $arrHeader = $arrHeader;
        }

        if( ( !empty(session(SANDC_KEY.'.id'))||!empty(session(SANDC_KEY.'.userid')) ) && empty($arrHeader['Userid']) ){
            $arrHeader['Userid']    = empty(session(SANDC_KEY.'.userid'))?session(SANDC_KEY.'.id'):session(SANDC_KEY.'.userid');
            $arrHeader['Logintime'] = session(SANDC_KEY.'extraInfo.logintime');
            $arrHeader['Checkcode'] = session(SANDC_KEY.'extraInfo.checkcode');
        }
