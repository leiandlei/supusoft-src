...
class Core_Lib_Page
{
	...
	public function __construct($data = array())
	{
		...
		$this->list_rows 		= !empty($data['list_rows']) && $data['list_rows'] <= 2000 ? $data['list_rows'] : 15;
	...
	protected function _get_link($page,$text)
	{
		switch ($this->method) {
			...
			case 'html':
				...
				$url = str_replace('.php', '.php?page='.$page,$this->parameter);
			...
			default:
				...
				return '<li><a href="' . $script . '?' . $query_string . '">' . $text . '</a></li>' . "\n";
			break;
		}
	}
    ...
    protected function show_2()
    {
    	...
    	{
    		// $return = "<form  id='page_submit' method='post' action='{$_SERVER['PHP_SELF']}{$this->parameter}'>   ";
			...
			// exit; total_pages
    			$return = '';
    		// $return = "共{$this->total_rows}条记录,{$this->now_page}/{$this->total_pages} ";
    		// $return = "共{$this->total_rows}条记录,{$this->now_page}/{$this->total_pages} ";
    		$return .= $this->up_page('<');
			...
			q
			到第 <input style='height:auto;width:auto;'  type='text' value='{$nCurrent}' size='2'    name='p' title='指定页码'> 页 　

				<a  class='now_page s color2' title='指定页码'    onclick=document.getElementById('page_submit').submit(); > 跳转 </a>
			</form>
			" ; */
			///  先不搞了 ，，  有时间在弄这个 .... ＠lastModifyTime　2014-07-23
    ...
    protected function show_1()
    {
    	...
    	for ($i = $begin; $i <= $begin + $plus * 2;$i++)
    	{
    		...
    		if($i == $this->now_page)
    		{
    			$return .= "<li><a class='now_page'>$i</a></li>\n";
    		}
    		else
    		{
    			$return .= $this->_get_link($i, $i) . "\n";
