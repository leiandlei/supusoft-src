...
$tx         =$db->get_row("SELECT cti_code FROM `sp_project` WHERE `tid` = '$tid' AND `iso`='A02' AND `deleted` = '0'");		
extract( $tx, EXTR_SKIP );
// 审核标准
$audit_ver  =$db->get_var("SELECT audit_ver FROM `sp_project` WHERE `tid` = '$tid' AND `iso`='A02' AND `deleted` = '0'");
//项目编号
...
$filename = '审表002-2 EMS审核方案策划与实施记录('.$ep_name.').doc';
//读入模板文件 
$tpldata  = readover( DOCTPL_PATH . 'doc/SH-002-2.xml' );
//读入模板文件 
if($audit_ver =='A020101')
	{
		$tpldata = readover( DOCTPL_PATH . 'doc/SH-002-2.xml' );
	}
	
if($audit_ver =='A020102')
	{
		$tpldata = readover( DOCTPL_PATH . 'doc/SH-002-2-2.xml' );
	}
