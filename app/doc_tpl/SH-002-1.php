...
$iso  = $db->getAll("SELECT iso FROM `sp_project` WHERE `ct_id` = '$ctid' AND `deleted` = '0' group by iso");
$tx   = $db->get_row("SELECT cti_code FROM `sp_project` WHERE `tid` = '$tid' AND `iso`='A01' AND `deleted` = '0'");		
extract( $tx, EXTR_SKIP );		

// 审核标准
$audit_ver=$db->get_var("SELECT audit_ver FROM `sp_project` WHERE `tid` = '$tid' AND `iso`='A01' AND `deleted` = '0'");
// $audit_basis=$db->get_var("SELECT audit_basis FROM `sp_settings_audit_vers` WHERE `audit_ver` = '$audit_ver' AND `deleted` = '0'");
//项目编号
...
$filename = '审表002-1 QMS审核方案策划与实施记录('.$ep_name.').doc';
//读入模板文件 
if($audit_ver == 'A010101')
	{
		$tpldata = readover( DOCTPL_PATH . 'doc/SH-002-1.xml' );
	}
if($audit_ver == 'A010102'||$audit_ver == 'A010103')
	{
		$tpldata = readover( DOCTPL_PATH . 'doc/SH-002-1-1.xml' );
	}
