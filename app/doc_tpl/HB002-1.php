...
//项目编号,申请范围
$xm=$db->get_row("SELECT cti_code,audit_code,scope,base_num,add_num,exc_num,xc_num,fxc_num,total_num,exc_clauses,mark,audit_ver FROM `sp_contract_item` WHERE `eid` = '$eid' AND iso='A01' AND `deleted` = '0'");
...
//cnas费用//其他费用
$a=$db->get_var("SELECT cc.cost FROM `sp_contract_cost` cc left join  `sp_contract_item` ci on cc.`ct_id`=ci.`ct_id` WHERE cc.`eid`= '$eid' AND cc.`cost_type`='1003' AND ci.`iso`='A01' AND ci.`mark`='01'");
$b=$db->get_var("SELECT cc.cost FROM `sp_contract_cost` cc left join  `sp_contract_item` ci on cc.`ct_id`=ci.`ct_id` WHERE cc.`eid`= '$eid' AND cc.`cost_type`='1003' AND ci.`iso`='A01' AND ci.`mark`='99'");

/**标准信息**/
$sql_auditVers_Info    = "select * from sp_settings_audit_vers where iso='A01' and audit_ver='".$audit_ver."'";
$result_auditVers_Info = $db->getOne($sql_auditVers_Info);
/**标准信息**/

$filename = ' 合表002-1 QMS认证申请通知单('.$ep_name.').doc';
//读入模板文件 
$output = readover( DOCTPL_PATH . 'doc/HB002-1.xml' );
