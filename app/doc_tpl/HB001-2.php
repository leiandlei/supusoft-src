...
//增减人日
$where .= " AND ct_id = '$ctid' AND iso='A02'";
...
// 合同编号
$ct=$db->get_row("SELECT ct_code,review_date,pre_date,wb_db,approval_user,approval_date,ps_user,zizhi_require,sum_jhd FROM `sp_contract` WHERE `eid` = '$eid' AND `deleted` = '0'");
// print_r($ct);exit;
...
//cnas费用
$a=$db->get_row("SELECT cc.cost FROM `sp_contract_cost` cc left join  `sp_contract_item` ci on cc.`ct_id`=ci.`ct_id` WHERE cc.`eid`= '$eid' AND ci.`iso`='A02' AND cc.`cost_type`='1003' AND ci.`mark`='01'");


//其他费用
$b=$db->get_row("SELECT cc.cost FROM `sp_contract_cost` cc left join  `sp_contract_item` ci on cc.`ct_id`=ci.`ct_id` WHERE cc.`eid`= '$eid' AND cc.`cost_type`='1003' AND ci.`iso`='A02' AND ci.`mark`='99'");

//风险等级
$fx=$db->get_row("SELECT `risk_level` FROM `sp_contract_item` WHERE ct_id= '$ctid' AND  iso='A02'");
// print_r($fx);exit;
...
//认证标准
$basis=$db->get_var("SELECT audit_basis FROM `sp_contract_item` ci left join `sp_settings_audit_vers` av on av.`audit_ver`=ci.`audit_ver` WHERE av.`deleted` = '0' and ci.ct_id='$ctid' and ci.iso='A02'");
...
$filename = ' 合表001-2 EMS认证申请评审表及组织审核通知单('.$ep_name.').doc';
//读入模板文件 
$tpldata = readover( DOCTPL_PATH . 'doc/HB001-2.xml' );
...
$output = str_replace( '{ps_user}', $ps_user, $output );
...
// print_r($rt['add_basis_check']);exit;
