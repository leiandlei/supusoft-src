<?php
$comment_select="";
$_query=$db->query("SELECT * FROM `sp_hr` WHERE `job_type` LIKE '%1004%' OR `job_type` LIKE '%1005%' OR `job_type` LIKE '%1006%' AND `deleted` = '0' AND `is_hire` = '1' ");
...
//标签样式	
	...
	$sp_type = empty($sp_type)?'0':$sp_type;
	${'sp_type_'.$sp_type} = " ui-tabs-active ui-state-active";
...
//审批时间
	...
	if( !$export ){
		$query = $db->query("SELECT p.sp_type,COUNT(*) total FROM sp_project p WHERE 1 $where GROUP BY p.sp_type");
		while( $rt = $db->fetch_array( $query ) ){
			$sp_type_total[$rt['sp_type']] = $rt['total'];
		}
		$pages = numfpage( $sp_type_total[$sp_type] );
	}

	$where .= " AND p.sp_type = '$sp_type'";

	$resdb = array();
	$sql = "SELECT $fields FROM sp_project p $join WHERE 1 $where ORDER BY t.te_date DESC $pages[limit]";
	// echo "<pre />";print_r($sql);exit;;exit;
	$query = $db->query( $sql );
	while( $rt = $db->fetch_array( $query )){
