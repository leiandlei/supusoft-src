...
$approval_hidden = 'hidden';
 ...
 $enterList = $db->get_row("SELECT code,ctfrom FROM sp_enterprises WHERE eid = '$eid'"); 
 $year = date('Y');
 $sum  = $db->get_var("SELECT COUNT(*) FROM sp_contract WHERE ct_code LIKE '%-$year-%'");
/*
$sum  = $db->getAll("SELECT ct_code FROM sp_contract WHERE ct_code LIKE '%-$year-%'");
    $c_sum=[];
    foreach ($sum as $key) {
        $c_sum[]=substr($key['ct_code'],19,3);
    }
    rsort($c_sum);
    $temp_num = 1001;
    $new_num = $c_sum[0] + $temp_num;
*/
    
 $temp_num = 1001;
 $new_num = $sum + $temp_num;
 $real_num = substr($new_num,1,3); //即截取掉最前面的“1”
  if($enterList['ctfrom']=='01000000')
 {
 	$type  = 'sc';
 }else{
 	$type  = 'wl';
 }
 $num = 'lll-'.$type.'-'.$enterList['code'].'-'.$year.'-'.$real_num;
...
if ($step) {
    ...
    $new_contract = array(
        ...
        'gltx_jhd'        => getgp('gltx_jhd'),
        'xtgc_jhd'        => getgp('xtgc_jhd'),
        'glps_jhd'        => getgp('glps_jhd'),
        'ns_jhd'          => getgp('ns_jhd'),
        'fzhmb_jhd'       => getgp('fzhmb_jhd'),
        'gjjz_jhd'        => getgp('gjjz_jhd'),
        'glzz_jhd'        => getgp('glzz_jhd'),
        'sum_jhd'         => getgp('sum_jhd'),
        'zizhi_require'   => getgp('zizhi_require'),
        'start_date'      => getgp('start_date')

    );
    // echo "<pre />";
    // print_r($new_contract);exit;
    ...
    $is_obtains  = @array_map('trim', getgp('is_obtain'));
    ...
    if ($ct_id) {
        ...
        while ($rt = $db->fetch_array($query)) {
            ...
            //是否获证
            $rt['is_obtain_0']                   = $rt['is_obtain_1'] = '';
            $rt['is_obtain_' . $rt['is_obtain']] = 'selected';
