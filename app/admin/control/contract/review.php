...
if( $step ){
    ...
    $xc_num       	   = getgp( 'xc_num' );//现场人日数
    $fxc_num     	   = getgp( 'fxc_num' );//非现场人日数  
	...
	$audit_codes       = getgp( 'audit_code_id' );
	$use_codes         = getgp( 'use_code' );
	$audit_codes_2017  = getgp( 'audit_code_id_2017' );//2017
	$use_codes_2017    = getgp( 'use_code_2017' );
	...
	if( $audit_codes )
	{
		...
		foreach( $audit_codes as $cti_id => $audit_code )
		{
			...
			$use_codes[$cti_id]   = join("；",array_unique(explode("；",$use_codes[$cti_id])));
			$cti_array = array(
				'base_num'		=> $base_num[$cti_id], 
				'total_num'		=> $total_num[$cti_id], 
				'ch_num'		=> $ch_num[$cti_id],
				'xc_num'		=> $xc_num[$cti_id], //现场人日数
				'fxc_num'		=> $fxc_num[$cti_id], //非现场人日数
	...
	if( $audit_codes_2017 )
	{
		foreach( $audit_codes_2017 as $cti_id => $audit_code_2017 )
		{
			$audit_code_2017      = str_replace(array(",","|",";","，"),"；",$audit_code_2017);
			$use_codes_2017[$cti_id]   = str_replace(array(",","|",";","，"),"；",$use_codes_2017[$cti_id]);
			$use_codes_2017[$cti_id]   = join("；",array_unique(explode("；",$use_codes_2017[$cti_id])));
			$cti_array = array
			(
				'use_code_2017'	    => $use_codes_2017[$cti_id], 
				'audit_code_2017'	=> $audit_code_2017, 
			);	

			$cti->edit( $cti_id, $cti_array, $status );
			// print_r($add_num[$cti_id]);exit;
		}
	}
/* 	$_codes=array();
 ...
 */		$rt['audit_ver_V']  = f_audit_ver( $rt['audit_ver'] );
	...
	//	if($rt[wenshen]){
		...
		//	$rt[display]="";
	//	}
		$rt['is_obtain_V'] = ($rt['is_obtain']) ? '是' : '否';
		$rt[display]="none";

		$ct_items[$rt['cti_id']] = $rt;
		$rt['audit_code']       = explode("；",$rt['audit_code']);
		
		$rt['audit_code_2017']  = explode("；",$rt['audit_code_2017']);
	...
	if( $_audit_codes )
	{
		...
		{
			$sql = "SELECT code,shangbao,risk_level,mark,banben,id FROM sp_settings_audit_code WHERE iso = '{$codes['iso']}' AND  id IN('".implode("','",$codes['audit_code_2017'])."')";
			$query = $db->query($sql);
			while( $rt = $db->fetch_array( $query ) ){
...
//		echo "<pre />";
//		print_r($audit_codes_2017);exit;
		foreach( $_audit_codes as $cti_id => $codes )
		{
			
			$sql = "SELECT code,shangbao,risk_level,mark,banben,id FROM sp_settings_audit_code WHERE iso = '{$codes['iso']}' AND  id IN('".implode("','",$codes['audit_code'])."')";
			$query = $db->query($sql);
			while( $rt = $db->fetch_array( $query ) ){
	...
	!$review_date && $review_date=$accept_date;
