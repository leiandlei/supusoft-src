...
$is_check = getgp('is_check');
$is_check = empty($is_check)?'n':'y';
${'status_'.$is_check.'_tab'} =' ui-tabs-active ui-state-active';

$where .= " AND cert.status <> '' and cert.is_check='".$is_check."' ";
$certstate_select = str_replace("value=\"$certstate\">","value=\"$certstate\" selected>",$certstate_select);
	...
	$join .= " left join sp_project p on p.cti_id = cert.cti_id ";
	$where.= "and p.tid!='' and p.deleted=0 AND p.ifchangecert = '2'";
	$where.= " and cert.deleted=0";
...
if( !$export and !$export1){
	...
	$totaln = $db->get_var("SELECT COUNT(*) FROM sp_certificate cert $join WHERE 1 ".preg_replace("/cert.is_check='[y\|n]'/","cert.is_check='n'",$where));

	$totaly = $db->get_var("SELECT COUNT(*) FROM sp_certificate cert $join WHERE 1 ".preg_replace("/cert.is_check='[y\|n]'/","cert.is_check='y'",$where));
	$pages = numfpage( ${'total'.$is_check} );
}

$sql       = "SELECT p.id as pid,cert.* FROM sp_certificate cert $join WHERE 1 $where ORDER BY cert.id DESC $pages[limit]";
$query     = $db->query( $sql );
while( $rt = $db->fetch_array( $query ) ){
