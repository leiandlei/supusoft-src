...
if( !$export and !$export1){
	...
	$totaln = $db->get_var("SELECT COUNT(*) FROM sp_certificate cert WHERE 1 ".preg_replace("/cert.is_check='[y\|n]'/","cert.is_check='n'",$where));
	$totaly = $db->get_var("SELECT COUNT(*) FROM sp_certificate cert WHERE 1 ".preg_replace("/cert.is_check='[y\|n]'/","cert.is_check='y'",$where));
