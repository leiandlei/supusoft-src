...
//换证原因
	...
	while($p_info = $db->fetch_array($res)){
		...
		$addr['prod_addr']   = $p_info['prod_addr'];//其他地址
		// print_r($p_info['prod_addr']);exit;
		$addr['prod_addr_e'] = $p_info['prod_addr_e'];//其他地址英文
		$addr['prod_check']  = $arr_addr_prod_check[$p_info['prod_check']];
	}
	...
	if( empty($addr) ){
        ...
        $prod_check        = str_replace('\"','"',$en_info['prod_check']);
		$prod_check        = unserialize($en_info['prod_check']);//类型
