...
while ($rt = $db->fetch_array($query)) {
    ...
    $auditType = ($rt['audit_type']==1002)?'1003':$rt['audit_type'];
    $sql_cost="SELECT `status` FROM sp_contract_cost WHERE `ct_id`=".$rt['ct_id']." AND cost_type='".$auditType."' and deleted=0";
    $query_cost = $db->query($sql_cost);
    $status = $db->fetch_array($query_cost);
    if( empty($status) ){
        $projects[$rt['id']]['cost_status']='未登记';
    }else{
        if( $status['status']==0 ){
            $projects[$rt['id']]['cost_status']='未缴费';
        }else{
            $projects[$rt['id']]['cost_status']='已缴费';
        }
    }
}
