...
while ($rt = $db->fetch_array($query)) {
    //$project_codes[$rt['id']] = explode('；', str_replace( array('；',';'), '；', $rt['audit_code'] ) );
    $project_codes[$rt['id']] = explode('；', str_replace(array(
        '；',
        ';'
    ), '；', $rt['use_code']));
    $ck_is_leaders[$rt['id']] = false;
    $xq_codes[$rt['id']]      = f_audit_ver($rt['audit_ver']) . '：' . $rt['use_code'];
    $ck_codes[$rt['id']]      = f_audit_ver($rt['audit_ver']) . '：';
    $xq_is_leaders[$rt['id']] = f_audit_ver($rt['audit_ver']) . '：是';
    //是否组长
    $role_1001                = $db->get_var("SELECT COUNT(*) FROM  sp_task_audit_team  WHERE tid = '$tid' AND pid = '$rt[id]' AND role='1001' and deleted=0 ");
    $is_leaders               = ($role_1001) ? ('有') : ('无');
    if (!$role_1001) {
        $ck_is_leader = 'error';
    }
    ...
    $resdb[$rt['id']]         = $rt;
    $project_codes[$rt['id']] = trim($rt['use_code']);
}
//人日
$_num  = 0.0;
...
while ($rt = $db->fetch_array($query)) {
    ...
    $rt['use_code'] = str_replace(';', '；', $rt['use_code']);
    $_codes         = explode('；', $rt['use_code']); //分解审核员代码到数组
