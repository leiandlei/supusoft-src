 ...
 */		$rt['audit_ver_V'] = f_audit_ver( $rt['audit_ver'] );
	...
	if( $_audit_codes ){
		foreach( $_audit_codes as $cti_id => $codes ){
			$sql = "SELECT code,shangbao,risk_level,mark FROM sp_settings_audit_code WHERE iso = '{$codes['iso']}' AND  shangbao IN('".implode("','",$codes[audit_code])."') and deleted=0 and is_stop=0 ";
			$query = $db->query($sql);
			while( $rt = $db->fetch_array( $query ) ){
				$marks = explode( ',', $rt['mark'] );
				$new_marks = array();
