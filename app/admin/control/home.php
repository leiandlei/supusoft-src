...
if ($_SESSION['extraInfo']['userType']=="stuff") {
	...
	//公司公告
	$notices = array();
	$totals=$db->get_var("SELECT COUNT(*) FROM sp_notice  WHERE status=1 and type=1");
	$pages = numfpage( $totals, 8);
	$query = $db->query( "SELECT n.*,hr.name author FROM sp_notice n INNER JOIN sp_hr hr ON hr.id = n.create_uid WHERE n.status=1 and n.type=1 ORDER BY id DESC $pages[limit]" );
	...
	//业务流转
	$send = array();
	$query = $db->query( "select * from sp_notice where deleted=0  and type=11 ORDER BY id DESC $pages[limit]" );
	while( $rt = $db->fetch_array( $query ) )
	{
		$receiveuser = explode(',', $rt['receiveuser']);
		if($userid !='1')
		{
			if(in_array($userid, $receiveuser))
			{
				$rt['filename'] = substr($rt['filename'],strlen($rt['id'].'_') );
				$send[] = $rt;
				$total++;
...
//业务流转结束
	//if( check_sys_boolen( 'notice' ) ){
		...
		//审核项目.未上报
		$project_total1 = array(0,0);
		$query = $db->query("SELECT status_sb,COUNT(*) total FROM sp_task   WHERE 1 AND status_sb=0  and deleted=0 GROUP BY status_sb");
		while( $rt = $db->fetch_array( $query ) ){
			$project_total1[$rt['status_sb']] = $rt['total'];
		}
		//认证评定
...
//@zbzytech 企业看到的公告不同
    if( 'yes' == $_SESSION['userinfo']['is_customer'] ) tpl('customer_home');   
    else tpl('home');
?>