...
*/
    ...
    //取任务的 体系、专业代码
    $t_isos = $t_use_codes = $t_projects =$_audit_code_arr= array();
    ...
    while ($rt = $db->fetch_array($query)) {
        ...
        $t_use_codes[$rt['iso']] = $rt['use_code'];
        $_audit_code_arr[$rt['iso']] = $rt['audit_code'];
        $t_projects[] = $rt;
    };
    //人员性质下拉
    $audit_job_select = '';
        ...
        if ($audit_codes[$rt['iso']]) {
           $query2 = $db->query("SELECT use_code FROM sp_hr_audit_code WHERE iso = '$rt[iso]' AND uid = '$rt[uid]' AND use_code IN ('" . str_replace(array(
            ...
            while ($rt2 = $db->fetch_array($query2)) {
                $_audit_codes[] = $db->get_var("SELECT shangbao FROM `sp_settings_audit_code` WHERE `code` = '$rt2[use_code]' AND `shangbao` IN ('" . str_replace(array(
                '；',
                ';',
                ' '
            ) , "','", $_audit_code_arr[$rt['iso']]) . "')");
                $_use_codes[] = $rt2['use_code'];
            }
			
        }
		
        // $_use_codes && $rt['audit_code'] = $_audit_code_arr[$rt[iso]];
