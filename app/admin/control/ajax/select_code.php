...
*/
	...
	(empty($banben))?$banben='2':$banben=$banben;
    $where   = '';
    if ($iso) {
        $where.= " AND iso = '$iso' ";
    }
    ...
    $codes = array();
    $total = $db->get_var("SELECT COUNT(*) FROM sp_settings_audit_code WHERE 1 $where and banben=2 and deleted=0 and is_stop=0 ");
    $pages = numfpage($total, 40);
    $query = $db->query("SELECT * FROM sp_settings_audit_code WHERE 1 $where and deleted=0 and banben=2 and  is_stop=0 $pages[limit]");
