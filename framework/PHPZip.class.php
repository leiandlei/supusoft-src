<?php
    class PHPZip
    {
        ...
        //     printf("%s<br>\r\n", $file);
        // ------------------------------------------------------ //
        public function visitFile($path,$fileList = array())
        {
            ...
            while(($file = @$fdir->read()) !== false)
            {
                ...
                if(is_dir($pathSub)){ $this->visitFile($pathSub,$fileList = array()); }
            }
            $fdir->close();
            return $fileList;
        }
        ...
        *   @$overwrite 是否为覆盖与目标文件相同的文件  
        ...
        function create_zip($files = array(),$destination = '',$down=true,$overwrite = false) {  
                ...
                foreach($files as $file) {
                    if(is_array($files)){
                        if(file_exists(iconv("UTF-8","GB2312",$file['url']))) {    
                            $valid_files[] = array(
                                    'name' => iconv("UTF-8","GB2312",$file['code'].$file['name'])
            ...
            if(count($valid_files)) {  
                ...
                //如果不是目录就创建目录
                if(!@is_dir(pathinfo($destination,PATHINFO_DIRNAME).'/'))@mkdir(pathinfo($destination,PATHINFO_DIRNAME).'/', 0777,true);
         ...
         */
        public function downBig($sourceFile='',$outFile='',$del=false){
            $file_extension = strtolower(substr(strrchr($sourceFile, "."), 1)); //获取文件扩展名 
            $outFile        = $outFile.'.'.$file_extension; 
            //echo $sourceFile;  
