...
{
    public $_tb = 'training'; //企业表名
    public $_pk = 'tid'; //企业表主键
    function add($args)
    {
        global $db;
        $eid = $db->insert('training', $args);
        //处理附加属性
        // if (getgp('meta'))
        ...
        //         $ADDSQL[] = "( '$tid', '$meta', '$value', 'training' )";
        //     }
        //     if ($ADDSQL) {
    ...
    // function union_count($tid, $number = 0)
    // {
    //     return $this->_count($tid, 'union', $number);
    // }

    // function site_count($tid, $number = 0)
    // {
    //     return $this->_count($tid, 'site', $number);
    // }

    // function _count($tid, $field, $number = 0)
    // {
    //     if (empty($tid) || empty($field))
    //         return false;
    //     global $db;
