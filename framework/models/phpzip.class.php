...
class PHPZip{
	...
	public function	zipDown( $files, $names = array() ){
		if(@!function_exists('gzcompress')){ return; }
		...
		//@HBJ 2013年9月12日 10:39:29修正打包重名覆盖问题
		$nameArr_tmp = array();
		for( $i = 0; $i < $file_num; $i++ ){
			...
			$name = isset( $names[$i] ) ? $names[$i] : basename( $file );
			
			$count = count(array_keys($nameArr_tmp, $name));
			$nameArr_tmp[] = $name;
			if($count) {
				$name = pathinfo($name, PATHINFO_FILENAME) . "($count)." . pathinfo($name, PATHINFO_EXTENSION);
			}
			$this->addFile( $content, $name );
		}
		...
		@header('Content-Encoding: none');
		@header('Content-Type: application/zip');
		@header('Content-Disposition: attachment ; filename=ZipFile_'.date("Y-m-d_His", time()).'.zip');
		@header('Pragma: no-cache');
		@header('Expires: 0');
		print($out);
	}
